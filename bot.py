...
def start(m, res=False):
    bot.send_message(m.chat.id, 'Привет, это бот kpop idols 💜 я помогаю найти айдола с наибольшей совместимостью, '
                                'напиши /info1, чтобы узнать о проекте, /reg, чтобы начать, /get_info, чтобы узнать про '
                                'айдола или /getwiki, чтобы посмотреть информацию про айдола из википедии')
...
def start2(message):
    if message.text == '/reg':
        global idol
        idol = df_final
        bot.send_message(message.from_user.id, "Напиши, пожалуйста, месяц твоего рождения (одну цифру, если твой месяц "
                                               "<10)")
        bot.register_next_step_handler(message, get_month)
    elif message.text == '/get_info':
        bot.send_message(message.from_user.id, 'Напиши на русском языке с большой буквы имя айдола, '
                                               'про которого хочешь узнать, '
        ...
        if ddd >= pd.to_datetime(y + '-03-21') and ddd <= pd.to_datetime(y + '-04-20'):
            idol = oven[oven['year of birth'] == oven['year_gap'].min()]
        ...
        bot.send_message(message.from_user.id, f"Твой айдол 🧙: {str(idol['Stage Name'])} из {str(idol['Group'])}")
        bot.send_message(message.from_user.id, f"Посмотри наш сайт с информацией про kpop idol 💜: "
                                               f"https://share.streamlit.io/foxiiny/fp/main/fp.py")
        ...
        bot.send_message(message.from_user.id, 'Данный бот является частью финального проекта по data science. '
                                               'Бот написан с использованием библиотеки telebot. '
                                               'В функции /getwiki используются регулярные выражения.'
                                               'В функции /get_info используется web scrapping с помощью библиотеки '
                                               'BeautifulSoup ✨')
    else:
...
def getwiki(message):
    try:
        ...
        bot.send_message(message.from_user.id,
                         f"Посмотри наш сайт с информацией про kpop idol 💜: "
                         f"https://share.streamlit.io/foxiiny/fp/main/fp.py")
...
def get_month(message):
    ...
    try:
        ...
        bot.register_next_step_handler(message, get_month)
    bot.send_message(message.from_user.id, 'Напиши, пожaлуйста, день твоего рождения (одну цифру, если день <10)')
...
def get_day(message):
    ...
    try:
        ...
        bot.send_message(message.from_user.id, 'Введи, пожалуйста, цифры. Напиши /reg, чтобы начать заново')
        bot.register_next_step_handler(message, get_day)
    bot.send_message(message.from_user.id, 'Напиши, пожалуйста, год твоего рождения')
...
def get_info(message):
    ...
    if len(g) !=0:
        ...
        bot.send_message(message.from_user.id,
                         f"Посмотри наш сайт с информацией про kpop idol 💜: "
                         f"hhttps://share.streamlit.io/foxiiny/fp/main/fp.py")
    else:
